...
---
user-invocable: true
description: 'Run linting tools on changeset and provide feedback'
...
- Check whether `make` exists (`command -v make`). If it is missing, report a blocker.
...
Always prefer Makefile targets over calling Black/Ruff directly.


The intention is to run linting tools on only modified files within the changeset.
you should only run linting tools on files that are modified within the changeset. This ensures that you are not making formatting or linting changes to files that were not modified by the developer, which could create noise in the pull request and make it harder for reviewers to focus on the actual changes made by the developer.

For the changeset in question, run these subagents sequentially:
- black formatter via `make format`
- ruff linter via `make ruff-check` and `make ruff-format`
